...
namespace Pes.Core
{
    public partial class Comment
    {
        ...
        public static List<Comment> GetCommentsBySystemObject(int SystemObjectID, long SystemObjectRecordID, int Skip)
        {
            List<Comment> results =
                All()
                .Where(c => c.SystemObjectID == SystemObjectID && c.SystemObjectRecordID == SystemObjectRecordID)
                .OrderByDescending(c => c.CreateDate).Skip(Skip)
                .ToList();
            return results;
        }
        public static List<Comment> GetTopCommentsBySystemObject(int SystemObjectID, long SystemObjectRecordID, int Take)
        {
            List<Comment> results =
                All()
                .Where(c => c.SystemObjectID == SystemObjectID && c.SystemObjectRecordID == SystemObjectRecordID)
                .OrderByDescending(c => c.CreateDate).Take(Take)
                .ToList();
            return results;
        }
