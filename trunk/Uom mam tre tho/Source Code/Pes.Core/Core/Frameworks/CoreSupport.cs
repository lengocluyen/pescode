...
{
    public CoreSupport() { }
    public static int StringCount(string s, char i)
    {
        int count = 0;
    ...
    public static string EncodePassword(string strPassword, string strFormat)
    {
        ...
        return FormsAuthentication.HashPasswordForStoringInConfigFile(strPassword + "ITBUSTEAM", strFormat);
    }
    /// <summary>
    ...
    {
        return HttpContext.Current.Request.ServerVariables["HTTP_HOST"] + HttpContext.Current.Request.ApplicationPath;
    }
    ...
    public static string GetImageURL(object imgURL)
    {
        ...
        if (currentURL == string.Empty)
            urlResult = ForceString(ConfigurationSettings.AppSettings.Get("HttpProtocol")) + GetHostAndApplicationPath() + ForceString(ConfigurationSettings.AppSettings.Get("NoImage"));
        else
            urlResult = ForceString(ConfigurationSettings.AppSettings.Get("HttpProtocol")) + GetHostAndApplicationPath() + ForceString(imgURL);
    ...
    {
        string urlResult = null;
        urlResult = ForceString(ConfigurationSettings.AppSettings.Get("HttpProtocol")) + GetHostAndApplicationPath();
        return urlResult;
    }
    ...
    public static string callAudioPlayer(object sourceAudio, int widthIp)
    {
        string audioControl = "<object height='40' align='top' width='" + widthIp.ToString() + "' id='sonpamp3player' codebase='http://fpdownload.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=6,0,0,0'"
            ...
            + ForceString(ConfigurationSettings.AppSettings.Get("HttpProtocol")) + GetHostAndApplicationPath() + ForceString(sourceAudio) + "name='flashVars' />"
            + "<param value='" + ForceString(ConfigurationSettings.AppSettings.Get("HttpProtocol")) + GetHostAndApplicationPath() + "/Swf/player.swf" + "' name='movie' />"
            ...
            + ForceString(ConfigurationSettings.AppSettings.Get("HttpProtocol")) + GetHostAndApplicationPath() + ForceString(sourceAudio)
            + "'src='" + ForceString(ConfigurationSettings.AppSettings.Get("HttpProtocol")) + GetHostAndApplicationPath() + "/Swf/player.swf" + "' style='margin: 0px 0pt 0px 0px;"
