...
using System.Text;
using ITBusSilverlight;
using TracNghiem.ITBusServiceRefernce;
using System.Collections.Generic;
using System.Windows.Browser;
using Liquid;
using System.Windows.Media.Imaging;
using TracNghiem.PESServicesSession;
...
namespace TracNghiem
{
    public partial class MainPage : UserControl
    {
        ...
        IList<VQuestions> questions;    // Dung de luu tru tat ca cac cau hoi theo Level.
        ...
        {
            if (ITBusSilverlightCommons.parameters["levelID"] != null)
            {
                try
                {
                    levelID = int.Parse(ITBusSilverlightCommons.parameters["levelID"].ToString());
                    questionsNumber = int.Parse(ITBusSilverlightCommons.parameters["questionsNumber"].ToString());
                    minutes = int.Parse(ITBusSilverlightCommons.parameters["timeExpire"].ToString()) - 1;
                    seconds = 60;


                    getSession.GetUserLoginIDCompleted += new EventHandler<GetUserLoginIDCompletedEventArgs>(getSession_GetPupilLoginIDCompleted);
        ...
        void web_GetAllQuestionByTestLevelCompleted(object sender, GetAllQuestionByTestLevelCompletedEventArgs e)
        {
            ...
            if (questions != null)
            {
                if (questions.Count > 0)
                {
                    ...
                    {
                        foreach (VQuestions ques in questions)
                        {
        ...
        void web_GetQuestionByIDCompleted(object sender, GetQuestionByIDCompletedEventArgs e)
        {
            if (loadQuestion)
            {
                ...
                //Sound Event
                if (question.SoundFile != null)
                {
                    QuestionSound.Source = new Uri(webURL + question.SoundFile, UriKind.RelativeOrAbsolute);
                    QuestionSound.Play();
                }
                loadQuestion = false;
        ...
        void web_GetAllAnswersByQuestionCompleted(object sender, GetAllAnswersByQuestionCompletedEventArgs e)
        {
            ...
            if (list.Count > 0)
            {
                ...
                int stt = 1;
                foreach (VAnswers an in list)
                {
                    StackPanel stack = new StackPanel();
                    stack.Name = "answerStack_" + an.AnswerID.ToString();
                    ...
                    if ((bool)an.IsText)
                    {
                        ...
                        if (!TestAnswerIsActive(an.AnswerID.ToString()))
                            answerText.Style = (Style)this.Resources["AnswerStyle"];
                        else
                            answerText.Style = (Style)this.Resources["AnswerHoverStyle"];
                        answerText.Name = "answer_" + an.AnswerID.ToString() + "_" + an.IsTrue.ToString();
                        answerText.Text = an.AnswerContent;
                    ...
                    else
                    {
                        ...
                        answerImg.Name = "answer_" + an.AnswerID.ToString() + "_" + an.IsTrue.ToString();
                        MessageBox.Show(webURL + an.AnswerContent);
                        answerImg.Source = new BitmapImage(new Uri(webURL + an.AnswerContent, UriKind.RelativeOrAbsolute));
        ...
        void web_InsertTestResultCompleted(object sender, InsertTestResultCompletedEventArgs e)
        {
            if (e.Result)
            {
                if (levelID == pfile.LevelOfExperienceTypeID)
                {
                    ...
                    web.UpdateProFileCompleted += new EventHandler<System.ComponentModel.AsyncCompletedEventArgs>(web_UpdateProFileCompleted);
        ...
        void web_UpdateProFileCompleted(object sender, System.ComponentModel.AsyncCompletedEventArgs e)
        {
            throw new NotImplementedException();
        }

    
        void web_UpdatePupilCompleted(object sender, System.ComponentModel.AsyncCompletedEventArgs e)
        {
            isUpLevel = true;
            ShowPopup();
        }
        ...
        private void InsertTestResult()
        {
            try
            {
                ...
                if (a > 0.7)
                {
                    VTestResults testResult = new VTestResults();
                    testResult.Mark = (float)markResult;
                    testResult.TestDate = DateTime.Now;
                    testResult.PupilID = pupilID;
